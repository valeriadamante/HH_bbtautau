    ...
    struct FitResults {
        ...
        FitResults()
            : has_valid_momentum(false),
              transverseMass(std::numeric_limits<double>::lowest()),
              transverseMass_error(std::numeric_limits<double>::lowest()) {}
        FitResults(bool _has_valid_momentum,
                   LorentzVectorM _momentum,
                   LorentzVectorM _momentum_error,
                   double _transverseMass,
                   double _transverseMass_error)
            : has_valid_momentum(_has_valid_momentum),
              momentum(_momentum),
              momentum_error(_momentum_error),
              transverseMass(_transverseMass),
              transverseMass_error(_transverseMass_error) {}
    };
    class FitProducer {
      public:
        static classic_svFit::MeasuredTauLepton CreateMeasuredLepton(const LorentzVectorM& momentum,
                                                                     Leg leg_type,
                                                                     int decay_mode) {
            ...
            return classic_svFit::MeasuredTauLepton(
                decay_type, momentum.Pt(), momentum.Eta(), momentum.Phi(), preciseVisMass, decay_mode);
        }
        static FitResults Fit(const LorentzVectorM& tau1_p4,
                              Leg tau1_leg_type,
                              int tau1_decay_mode,
                              const LorentzVectorM& tau2_p4,
                              Leg tau2_leg_type,
                              int tau2_decay_mode,
            ...
            if (algo.isValidSolution()) {
                auto histoAdapter =
                    dynamic_cast<classic_svFit::DiTauSystemHistogramAdapter*>(algo.getHistogramAdapter());
                result.momentum = LorentzVectorM(
                    histoAdapter->getPt(), histoAdapter->getEta(), histoAdapter->getPhi(), histoAdapter->getMass());
                result.momentum_error = LorentzVectorM(histoAdapter->getPtErr(),
                                                       histoAdapter->getEtaErr(),
                                                       histoAdapter->getPhiErr(),
                                                       histoAdapter->getMassErr());

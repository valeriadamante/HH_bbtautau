...
if __name__ == "__main__":
    ...
    ########## dataFrame creation ##########
    sample_list = (
        ggR_samples + ggBG_samples
    )  # if args.sample == 'GluGluToRadion' else ggBG_samples
    inFiles = []
    ...
    for pfreliso_th in [
        ...
        df = AnaBaseline.RecoHttCandidateSelection_ForEfficiency(
            df, setup.global_params, pfreliso_th
        )
        df = df.Define("recochannelId", "static_cast<int>(HttCandidate.channel())")
        df_den = df.Filter("recochannelId==22 || recochannelId==23")
        print(
            f"events in reco channels containing a muon are: {df_den.Count().GetValue()}"
        )
        df_num = df_den.Filter("genchannelId==recochannelId").Count().GetValue()
        print(
            f"events in reco channels containing a muon matching to gen channel are: {df_num.Count().GetValue()}"
        )

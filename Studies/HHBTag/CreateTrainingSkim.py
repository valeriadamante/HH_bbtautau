...
def JetSavingCondition(df):
    df = df.Define(
        "Jet_selIdx", "ReorderObjects(Jet_btagDeepFlavB, Jet_idx[Jet_bCand])"
    )
...
def createSkim(
    inFile, outFile, period, sample, X_mass, node_index, mpv, config, snapshotOptions
):
    ...
    df = df.Filter(
        "GenRecoMatching(*genHttCandidate, HttCandidate, 0.2)", "SameGenRecoHTT"
    )
    ...
    colToSave = [
        "event",
        "luminosityBlock",
        "HttCandidate_leg0_pt",
        "HttCandidate_leg0_eta",
        "HttCandidate_leg0_phi",
        "HttCandidate_leg0_mass",
        "HttCandidate_leg1_pt",
        "HttCandidate_leg1_eta",
        "HttCandidate_leg1_phi",
        "HttCandidate_leg1_mass",
        "channel",
        "sample",
        "period",
        "X_mass",

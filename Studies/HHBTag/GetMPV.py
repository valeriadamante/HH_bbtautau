...
def FindMPV(df):
    df = (
        df.Define("bJetp4", " GenJet_p4[GenJet_B1]")
        .Filter("bJetp4.size()==2")
        .Filter("ROOT::Math::VectorUtil::DeltaR(bJetp4[0],bJetp4[1])>0.4")
        .Define("Two_bGenJets_invMass", "(bJetp4[0]+bJetp4[1]).M()")
    )
    histo = df.Histo1D(
        ("Two_bGenJets_invMass", "Two_bGenJets_invMass", 801, -0.25, 400.25),
        "Two_bGenJets_invMass",
    ).GetValue()
...
def GetMPV(inFile):
    ...
    df = Baseline.DefineGenObjects(df, 125.0)
    df = df.Define(
        "GenJet_B1", "GenJet_pt > 20 && abs(GenJet_eta) < 2.5 && GenJet_b_PF"
    )

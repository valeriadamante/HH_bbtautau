...
def compute_eff(input_file):
    ...
    for channel in channels:
        ...
        ch_correct_cnt[channel] = df.Filter(
            f"channelId == {channel} && gen_correct"
        ).Count()
    ...
    for (
        ...
        cnt,
        correct_cnt,
    ) in sorted(out, key=lambda x: -x[1]):
        cum_cnt += cnt
        print(
            f"{channel}: cnt={cnt} ({cnt/total_cnt * 100:.2f}, {cum_cnt/total_cnt * 100:.2f}) correct={correct_cnt} ({correct_cnt/cnt * 100:.2f})"
        )

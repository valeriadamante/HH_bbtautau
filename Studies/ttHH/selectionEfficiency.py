...
def compute_eff(input_dir):
    ...
    with ROOT.TFile.Open(f"{input_dir}/nano_0.root") as tmp_file:
        n_total_notsel = (
            df_notsel.Count()
            if "EventsNotSelected" in tmp_file.GetListOfKeys()
            else 0.0
        )
  ...
  """,
    ...
    df = df.Define(
        "nLeptons",
        "Electron_pt[Electron_sel].size() + Muon_pt[Muon_sel].size() + Tau_pt[Tau_sel].size()",
    )
    ...
    for n in range(n_max):
        print(
            f"{n} leptons: {lep_cnt[n].GetValue()} ({lep_cnt[n].GetValue() / n_total * 100:.2f}%)"
        )
    for n in range(n_max):
        print(
            f"{n} light leptons: {llep_cnt[n].GetValue()} ({llep_cnt[n].GetValue() / n_total * 100:.2f}%)"
        )
    for n in range(n_max):
        print(
            f"{n} hadronic taus: {tau_cnt[n].GetValue()} ({tau_cnt[n].GetValue() / n_total * 100:.2f}%)"
        )
    print(
        f">=2 leptons with at least 1 hadronic tau: {sel_cnt.GetValue()} ({sel_cnt.GetValue() / n_total * 100:.2f}%)"
    )
    print(
        f">=2 leptons with no hadronic tau: {sel_lep_cnt.GetValue()} ({sel_lep_cnt.GetValue() / n_total * 100:.2f}%)"
    )
    print(
        f">=2 hadronic taus (no e/mu): {had_sel_cnt.GetValue()} ({had_sel_cnt.GetValue() / n_total * 100:.2f}%)"
    )
    print(
        f">=2 hadronic taus (with e/mu): {had_lep_sel_cnt.GetValue()} ({had_lep_sel_cnt.GetValue() / n_total * 100:.2f}%)"
    )

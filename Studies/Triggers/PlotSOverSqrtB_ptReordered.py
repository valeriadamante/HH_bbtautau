    ...
    for key in dict_to_consider.keys():
        ...
        for sig_key in dict_to_consider[key].keys():
            if sig_key == "all":
                continue
            print(
                f"""{key_split[1]}\t{key_split[2]}\t{sig_key}\t{str(dict_to_consider[key][sig_key]["sum"]).replace('.',',')}\t{str(dict_to_consider[key][sig_key]["error"]).replace('.',',')}"""
            )
...
def print_final_table(dict_to_consider):
    for key in dict_to_consider.keys():
        ...
        for idx in range(0, len(dict_to_consider[key]["masses"])):
            print(
                f"""{key_split[1]}\t{key_split[2]}\t{str(dict_to_consider[key]['masses'][idx]).replace('.',',')}\t{str(dict_to_consider[key]['s/sqrtB_01'][idx]).replace('.',',')}\t{str(dict_to_consider[key]['s/sqrtB_012'][idx]).replace('.',',')}"""
            )
...
if __name__ == "__main__":
    ...
    bckg_files_DY_0 = f"backgrounds_DY_0{suffix}_ptReordered.json"
    bckg_files_DY_1 = f"backgrounds_DY_1{suffix}_ptReordered.json"
    bckg_files_DY_2 = f"backgrounds_DY_2{suffix}_ptReordered.json"
    bckg_files_TT_0 = f"backgrounds_TT_0{suffix}_ptReordered.json"
    bckg_files_TT_1 = f"backgrounds_TT_1{suffix}_ptReordered.json"
    bckg_files_TT_2 = f"backgrounds_TT_2{suffix}_ptReordered.json"
    ...
    with open(
        os.path.join(json_dir, sig_files_bulk_graviton), "r"
    ) as sig_file_bulk_graviton:
    ...
    dict_sqrtB_012 = GetSqrtBTot(
        all_sums_dict, ["TT_0", "TT_1", "TT_2", "DY_0", "DY_1", "DY_2"]
    )
    ...
    for wp_key in dict_signals_graviton.keys():
        ...
        for sample_key in dict_signals_graviton[wp_key].keys():
            ...
            if wp_key not in dict_signals_graviton.keys():
                ...
                continue
            ssqrtb_01 = (
                dict_signals_graviton[wp_key][sample_key]["sum"]
                / dict_sqrtB_01[wp_key]["sum"]
            )
            ssqrtb_01_ref = (
                dict_signals_graviton[wp_key_ref][sample_key]["sum"]
                / dict_sqrtB_01[wp_key_ref]["sum"]
            )
            ...
            ssqrtb_012 = (
                dict_signals_graviton[wp_key][sample_key]["sum"]
                / dict_sqrtB_012[wp_key]["sum"]
            )
            ssqrtb_012_ref = (
                dict_signals_graviton[wp_key_ref][sample_key]["sum"]
                / dict_sqrtB_012[wp_key_ref]["sum"]
            )
        ...
        for sample_key in dict_signals_radion[wp_key]:
            ...
            if wp_key not in dict_signals_radion.keys():
                ...
                continue
            ssqrtb_01 = (
                dict_signals_radion[wp_key][sample_key]["sum"]
                / dict_sqrtB_01[wp_key]["sum"]
            )
            ssqrtb_01_ref = (
                dict_signals_radion[wp_key_ref][sample_key]["sum"]
                / dict_sqrtB_01[wp_key_ref]["sum"]
            )
    ...
    plt.savefig(f"ssqrtb_rel_grav_all_bckgs{suffix}_01_ptReordered.pdf")
    plt.clf()
    color_key = 0
    ...
    plt.ylabel("ratio")
    plt.subplots_adjust(right=0.7)
    plt.savefig(f"ssqrtb_rel_grav_all_bckgs{suffix}_012_ptReordered.pdf")
    plt.clf()
    color_key = 0
    ...
    plt.ylabel("ratio")
    plt.subplots_adjust(right=0.7)
    plt.savefig(f"ssqrtb_rel_rad_all_bckgs{suffix}_01_ptReordered.pdf")
    plt.clf()
    color_key = 0
    ...
    plt.ylabel("ratio")
    plt.subplots_adjust(right=0.7)
    plt.savefig(f"ssqrtb_rel_rad_all_bckgs{suffix}_012_ptReordered.pdf")
    plt.clf()

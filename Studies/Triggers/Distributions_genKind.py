...
def getHistogram(df, trg_reg_list, weight_expression, model, var):
    ...
    filter_expr += ")"
    histo = (
        df.Define("weight_for_histo", weight_expression)
        .Filter(filter_expr)
        .Histo1D(model, var, "weight_for_histo")
    )
    return histo
    ...
    for sample_name in signal_samples + bckg_samples:
        ...
        dataframes_channel = {
            ...
            "tautau_Tight_Medium": df_tautau_Medium.Filter(
                f"tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Tight.value}"
            ),
            ...
            "tautau_VTight_VTight": df_tautau_VTight.Filter(
                f"tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.VTight.value}"
            ),
        ...
        for channel in dataframes_channel.keys():
            ...
            model_1 = ROOT.RDF.TH1DModel(
                "tau1_gen_kind",
                "tau1_gen_kind",
                x_bins_vec.size() - 1,
                x_bins_vec.data(),
            )
            model_2 = ROOT.RDF.TH1DModel(
                "tau2_gen_kind",
                "tau2_gen_kind",
                x_bins_vec.size() - 1,
                x_bins_vec.data(),
            ...
            histo_genKind_1 = getHistogram(
                dataframes_channel[channel],
                trg_tauTau_list_application_fullOR,
                f"final_weight*weight_ditau_application*weight_singleTau_application*weight_MET_application",
                model_1,
                "tau1_gen_kind",
            )
            histo_genKind_2 = getHistogram(
                dataframes_channel[channel],
                trg_tauTau_list_application_fullOR,
                f"final_weight*weight_ditau_application*weight_singleTau_application*weight_MET_application",
                model_2,
            ...
            outFile = ROOT.TFile.Open(
                f"Studies/Triggers/files/genkind_distrib_{args.bckg_sel}/gen_kinds_{sample_name}_{channel}_VSeVVLoose.root",
                "RECREATE",
            )

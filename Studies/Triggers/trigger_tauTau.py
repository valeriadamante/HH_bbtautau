...
def GetTriggerWeight(trig_list):
    ...
    for trg in trig_list:
        if trg == "MET":
            continue
        weights_str_list.append(
            f"weight_tau1_TrgSF_{trg}_Central*weight_tau2_TrgSF_{trg}_Central"
        )
...
if __name__ == "__main__":
    ...
    inDir = (
        "/eos/home-k/kandroso/cms-hh-bbtautau/anaTuples/Run2_2018/v2_deepTau2p1_HTT/"
    )
    ...
    for sample_name in sample_list:
        ...
        dfWrapped_central = DataFrameBuilder(
            ROOT.RDataFrame("Events", inFile), args.deepTauVersion
        )
        PrepareDfWrapped(dfWrapped_central)
        df_tauTau = dfWrapped_central.df.Filter("tauTau").Filter(
            "tau1_gen_kind==5 && tau2_gen_kind==5"
        )
        ...
        nInitial_tauTau = (
            df_tauTau.Define("total_weight_string_denum", total_weight_string_denum)
            .Sum("total_weight_string_denum")
            .GetValue()
        )
        ...
        if args.wantTightFirstTau:
            df_tautau_vloose.Filter(
                "tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Tight.value}"
            )
            df_tautau_medium.Filter(
                "tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Tight.value}"
            )
            df_tautau_loose.Filter(
                "tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Tight.value}"
            )
        ...
        for channel in ["tautau_medium", "tautau_loose", "tautau_vloose"]:
            ...
            for reg_key, reg_exp in regions_expr_bigTau.items():
                dataframes_channel_bigTau[channel] = dataframes_channel_bigTau[
                    channel
                ].Define(reg_key, reg_exp)
        ...
        pass_singleTau_validity = (
            f"""HLT_singleTau && ({GetTrgValidityRegion("tauTau","HLT_singleTau")})"""
        )
        pass_MET_validity = (
            f"""HLT_MET && ({GetTrgValidityRegion("tauTau","HLT_MET")})"""
        )
        pass_ditau_validity = (
            f"""HLT_ditau && ({GetTrgValidityRegion("tauTau","HLT_ditau")})"""
        )

        ##### VALIDITY TRIGGERS, WEIGHTS #####
        trg_tauTau_list_validity_fullOR = [
            pass_ditau_validity,
            pass_singleTau_validity,
            pass_MET_validity,
        ]
        trg_tauTau_list_validity_diTau_singleTau = [
            pass_ditau_validity,
            pass_singleTau_validity,
        ]
        ...
        dataframes_channel[channel] = dataframes_channel[channel].Define(
            f"weight_ditau_validity",
            f" {pass_ditau_validity}?  weight_tau1_TrgSF_ditau_Central*weight_tau2_TrgSF_ditau_Central : 1.f ",
        )
        dataframes_channel[channel] = dataframes_channel[channel].Define(
            f"weight_singleTau_validity",
            f"""if({pass_singleTau_validity}){{
        ...
        dataframes_channel[channel] = dataframes_channel[channel].Define(
            "weight_singleTau_ditau_validity",
            "weight_ditau_validity*weight_singleTau_validity",
        )
        ...
        dataframes_channel[channel] = dataframes_channel[channel].Define(
            "weight_singleTau_ditau_application",
            "weight_ditau_application*weight_singleTau_application",
        )

        dataframes_channel_bigTau[channel] = dataframes_channel_bigTau[channel].Define(
            f"weight_ditau_application",
            f" {pass_ditau_application}?  weight_tau1_TrgSF_ditau_Central*weight_tau2_TrgSF_ditau_Central : 1.f ",
        )
        dataframes_channel_bigTau[channel] = dataframes_channel_bigTau[channel].Define(
        ...
        AddEfficiencyToDict(
            dataframes_channel[channel].Define(
                f"total_weight_string",
                f"{total_weight_string_denum}*weight_ditau_validity",
            ),
            trg_tauTau_list_validity_diTau,
            "tauTau",
            nInitial_tauTau,
            "eff_validity_diTau",
            eff_tautau_validity,
        ...
        AddEfficiencyToDict(
            dataframes_channel[channel].Define(
                f"total_weight_string",
                f"{total_weight_string_denum}*weight_singleTau_ditau_validity",
            ),
            trg_tauTau_list_validity_diTau_singleTau,
            "tauTau",
            nInitial_tauTau,
            "eff_validity_diTauSingleTau",
            eff_tautau_validity,
            eff_tautau_validity_errors,
            args.wantNEvents,
        )
        ...
        AddEfficiencyToDict(
            dataframes_channel[channel].Define(
                f"total_weight_string",
                f"{total_weight_string_denum}*weight_ditau_application",
            ),
            trg_tauTau_list_application_diTau,
            "tauTau",
            nInitial_tauTau,
            "eff_application_diTau",
            eff_tautau_application,
            eff_tautau_application_errors,
            args.wantNEvents,
        )
        ...
        AddEfficiencyToDict(
            dataframes_channel[channel].Define(
                f"total_weight_string",
                f"{total_weight_string_denum}*weight_singleTau_ditau_application",
            ),
            trg_tauTau_list_application_diTau_singleTau,
            "tauTau",
            nInitial_tauTau,
            "eff_application_diTauSingleTau",
            eff_tautau_application,
            eff_tautau_application_errors,
            args.wantNEvents,
        )
        ...
        AddEfficiencyToDict(
            ...
            "tauTau",
            nInitial_tauTau,
            "eff_application_diTau_singleTau",
            eff_tautau_application_bigTau,
            eff_tautau_application_bigTau_errors,
            args.wantNEvents,
        )
        ...
        AddEfficiencyToDict(
            dataframes_channel_bigTau[channel].Define(
                f"total_weight_string",
                f"{total_weight_string_denum}*weight_singleTau_ditau_application",
            ),
            trg_tauTau_list_application_diTau_singleTau,
            "tauTau",
            nInitial_tauTau,
            "eff_application_diTau_singleTau_bigTau",
            eff_tautau_application_bigTau,
            eff_tautau_application_bigTau_errors,
            args.wantNEvents,
        )

...
def print_final_table(dict_to_consider):
    for key in dict_to_consider.keys():
        for sig_key in dict_to_consider[key].keys():
            ...
            print(
                f"""{sig_key_split[1]}\t{sig_key_split[2]}\t{key}\t{str(dict_to_consider[key][sig_key]["sum"]).replace('.',',')}\t{str(dict_to_consider[key][sig_key]["error"]).replace('.',',')}"""
            )
...
def GetSqrtBTot(all_sums):
    ...
    for key in all_sums_dict["TT"].keys():
        ...
        if key not in dict_sqrtB:
            dict_sqrtB[key] = {}
        dict_sqrtB[key]["sum"] = math.sqrt(
            all_sums_dict["TT"][key]["sum"] + all_sums_dict["DY"][key]["sum"]
        )
...
if __name__ == "__main__":
    ...
    sig_files_bulk_graviton = (
        f"signals_bulk_graviton{suffix}_VSeVLoose_onlyElectrons.json"
    )
    sig_files_radion = f"signals_radion{suffix}_VSeVLoose_onlyElectrons.json"
    bckg_files_DY = (
        f"backgrounds_DY_{args.bckg_sel}{suffix}_VSeVLoose_onlyElectrons.json"
    )
    bckg_files_TT = (
        f"backgrounds_TT_{args.bckg_sel}{suffix}_VSeVLoose_onlyElectrons.json"
    )
    ...
    with open(
        os.path.join(
            json_dir,
            f"all_sums_final_{args.bckg_sel}{suffix}_VSeVLoose_onlyElectrons.json",
        ),
        "w",
    ) as final_file:

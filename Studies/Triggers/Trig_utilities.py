...
def get_regions_dict_ptreordered(
    channel, singleTau_pt_limits=[190, 190], met_th=180, bigtau=False
):
    regions = {}
    tau1new_eta_sel = {
    ...
    first_tau_sel_singleTau = first_tau_sel.format(
        singleTau_pt_limits[0], tau1new_eta_sel[channel]
    )
    second_tau_sel_singleTau = second_tau_sel.format(
        singleTau_pt_limits[1], tau2new_2p1
    )

    first_tau_sel_other = first_tau_sel.format(
        tau_pt_limits[0], tau1new_eta_sel[channel]
    )
    second_tau_sel_other = second_tau_sel.format(tau_pt_limits[1], tau2new_2p1)
    single_lepton_validity = first_tau_sel.format(
        tau_pt_limits[0], tau1new_eta_sel[channel]
    )
    ...
    cross_lepton_validity = (
        f"{cross_lepton_validity_first} && {cross_lepton_validity_second}"
    )
    ...
    if bigtau:
        if channel == "tauTau":
            ...
            regions["singleTau_region"] = (
                f"""(({first_tau_sel_singleTau}) || ({second_tau_sel_singleTau}))"""
            )
            ...
            regions["other_trg_region"] = (
                f"""(({first_tau_sel_other}) &&( {second_tau_sel_other})) && !({regions["singleTau_region"]})"""
            )
        else:
            # second_tau_sel_singleTau = second_tau_sel.format(singleTau_pt_limits[1], tau2_2p1)
            ...
            regions["other_trg_region"] = (
                f"""(({single_lepton_validity}) || ({cross_lepton_validity})) && !({regions["singleTau_region"]})"""
            )
        ...
        if channel == "tauTau":
            ...
            # first_tau_sel_singleTau = first_tau_sel.format(singleTau_pt_limits[0], tau1_eta_sel[channel])
            regions["other_trg_region"] = (
                f"""(({first_tau_sel_other}) && ({second_tau_sel_other}))"""
            )
            regions["singleTau_region"] = (
                f"""(({first_tau_sel_singleTau}) || ({second_tau_sel_singleTau}))&& !({regions["other_trg_region"]})"""
            )
            ...
            # second_tau_sel_singleTau = second_tau_sel.format(singleTau_pt_limits[1], tau2_2p1)
            regions["other_trg_region"] = (
                f"""(({single_lepton_validity}) || ({cross_lepton_validity}))"""
            )
            regions["singleTau_region"] = (
                f"""({second_tau_sel_singleTau}) &&!({regions["other_trg_region"]}) """
            )

    regions["MET_region"] = (
        f"""(met_pt > {met_th} && !( ({regions["other_trg_region"]}) || ( {regions["singleTau_region"]} ) ) )"""
    )
    return regions
...
def get_regions_dict(channel, singleTau_pt_limits=[190, 190], met_th=180, bigtau=False):
    ...
    first_tau_sel_singleTau = first_tau_sel.format(
        singleTau_pt_limits[0], tau1_eta_sel[channel]
    )
    second_tau_sel_singleTau = second_tau_sel.format(singleTau_pt_limits[1], tau2_2p1)
    ...
    second_tau_sel_other = second_tau_sel.format(tau_pt_limits[1], tau2_2p1)
    single_lepton_validity = first_tau_sel.format(
        tau_pt_limits[0], tau1_eta_sel[channel]
    )
    cross_lepton_validity_first = first_tau_sel.format(tau_pt_limits[1], tau1_2p1)
    cross_lepton_validity_second = second_tau_sel.format(tau_pt_limits[2], tau2_2p1)
    cross_lepton_validity = (
...
def GetTrgValidityRegion(
    channel, trg, singleTau_pt_limits=[190, 190], met_th=150, year="2018"
):
    ...
    if channel == "tauTau" and trg == "HLT_singleTau":
        trg_validity_regions[trg] = (
            f"(tau1_pt > {singleTau_pt_limits[0]} && abs(tau1_eta) < 2.1) || (tau2_pt>{singleTau_pt_limits[1]} && abs(tau2_eta) < 2.1 )"
        )
    return trg_validity_regions[trg]


def GetTrgValidityRegion_ptreordered(
    channel, trg, singleTau_pt_limits=[190, 190], met_th=150, year="2018"
):
    trg_validity_regions = {
    ...
    if channel == "tauTau" and trg == "HLT_singleTau":
        trg_validity_regions[trg] = (
            f"(tau1new_pt > {singleTau_pt_limits[0]} && abs(tau1new_eta) < 2.1) || (tau2new_pt>{singleTau_pt_limits[1]} && abs(tau2new_eta) < 2.1 )"
        )
    return trg_validity_regions[trg]


def GetSum(
    df, channel, trg_reg_list, weight_expression, weight_dict, weight_key, verbose=False
):
    filter_expr = f" ("
...
def makeplot(
    ...
    for i, (key, values) in enumerate(eff_dict.items()):
        ...
        ratio_errors = []
        for val, error, ref, err_ref, ratio in zip(
            values, errors, ratio_ref, ref_errors, ratio_values
        ):
            ratio_errors.append(
                ratio
                * (math.sqrt(math.pow(error / val, 2) + math.pow(err_ref / ref, 2)))
            )
        if verbose:
            print(key)
            for mass, ratio, value, err in zip(x_values, ratio_values, values, errors):
                print(
                    f"mass = {mass} \t value = {value} \t error = {err} \t ratio = {ratio}"
                )
        ax1.errorbar(
            x_values,
            values,
            yerr=eff_dict_errors[f"{key}_error"],
    ...
    xticks = [
        ...
        2500,
        3000,
    ]  # Esempio di numeri da visualizzare sull'asse logaritmico
    ax2.set_xticks(xticks)
    ax2.set_xticklabels([str(num) for num in xticks])

    figName = (
        f"Studies/Triggers/eff_{channel}_{sample}_{deepTauWP}_{deepTauVersion}{suffix}"
    )

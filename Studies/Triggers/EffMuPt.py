...
if __name__ == "__main__":
    ...
    dfWrapped_central = DataFrameBuilder(
        ROOT.RDataFrame("Events", inFiles_vec), args.deepTauVersion
    )
    ...
    pass_singleTau_application = (
        "(HLT_singleTau && singleTau_region && !(MET_region) && ! (other_trg_region))"
    )
    ...
    for trig in [pass_singleMu_validity, pass_singleMu50_validity]:
        color_plt = colors[k]
        label_plt = labels[k]
        model_hist_denumerator = ROOT.RDF.TH1DModel(
        ...
        model_hist_numerator = ROOT.RDF.TH1DModel(
        ...
        n = len(x_bins)
        outFile = ROOT.TFile(
            f"Studies/Triggers/Mu50_efficiencies/muPt50_{mass_string}.root", "RECREATE"
        )
        ...
        for i in range(0, histo_numerator.GetNbinsX() + 1):
            ...
            conf_interval = proportion_confint(
                numerator, denominator, alpha=0.05, method="normal"
            )

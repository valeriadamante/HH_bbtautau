...
def getTauWPWeight(df, working_point_tau1, working_point_tau2):
    ...
    df = df.Define(
        f"weight_TauID_new",
        f"""weight_tau1_TauID_SF_{working_point_tau1}*weight_tau2_TauID_SF_{working_point_tau2}""",
    )
...
if __name__ == "__main__":
    ...
    for sample_name in signal_samples + bckg_samples:
        ...
        for channel in dataframes_channel.keys():
            ...
            dataframes_channel[channel] = dataframes_channel[channel].Filter(
                f"tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSe >= {Utilities.WorkingPointsTauVSe.Loose.value}"
            )
            # print(dataframes_channel[channel].Count().GetValue())
            ...
            dataframes_channel[channel] = dataframes_channel[channel].Filter(
                f"tau2_idDeepTau{deepTauYear}{args.deepTauVersion}VSe >= {Utilities.WorkingPointsTauVSe.Loose.value}"
            )
            # print(dataframes_channel[channel].Count().GetValue())
            ...
            dataframes_channel[channel] = (
                ...
                .Define("sel_id", "int(tau1_gen_kind == 5) + int(tau2_gen_kind == 5)")
            )
            ...
            if sample_name in signal_samples:
                ...
                dataframes_channel[channel] = dataframes_channel[channel].Filter(
                    f"sel_id == {args.bckg_sel} && ( tau1_gen_kind == 1 || tau2_gen_kind == 1 || tau1_gen_kind == 3 || tau2_gen_kind == 3) "
                )
    ...
    with open(
        f"Studies/Triggers/files/signals_{sampleName}{suffix}_VSeLoose.json", "w"
    ) as f:
        json.dump(s_values, f, indent=4)
    if bckg_samples:
        with open(
            f"Studies/Triggers/files/backgrounds_{bckg_names}_{args.bckg_sel}{suffix}_VSeLoose_onlyElectrons.json",
            "w",
        ) as f:

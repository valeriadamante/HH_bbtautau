...
def getTauWPWeight(df, working_point_tau1, working_point_tau2):
    ...
    df = df.Define(
        f"weight_tau1new_TauID_SF_{working_point_tau1}",
    ...
    df = df.Define(
        f"weight_tau2new_TauID_SF_{working_point_tau2}",
    ...
    df = df.Define(
        f"weight_TauID_new",
        f"""weight_tau1new_TauID_SF_{working_point_tau1}*weight_tau2new_TauID_SF_{working_point_tau2}""",
    )
...
def getSumWeights(df, trg_reg_list, weight_expression):
    ...
    filter_expr += ")"
    sum_channel = (
        df.Define("weight_for_sum", weight_expression)
        .Filter(filter_expr)
        .Sum("weight_for_sum")
        .GetValue()
    )
    sum_error = math.sqrt(
        df.Define("weight_for_sum_error", f"{weight_expression}*{weight_expression}")
        .Filter(filter_expr)
        .Sum("weight_for_sum_error")
        .GetValue()
    )
...
def getRFactor(df):
    ...
    den = (
        df.Define("weight_0_btag", "weights_0*weight_bTagShapeSF")
        .Sum("weight_0_btag")
        .GetValue()
    )
    return num / den
...
if __name__ == "__main__":
    ...
    header_path_RootExt = os.path.join(
        os.environ["ANALYSIS_PATH"], "PlotKit/include/RootExt.h"
    )
    ...
    inDir = (
        "/eos/home-k/kandroso/cms-hh-bbtautau/anaTuples/Run2_2018/v3_deepTau2p1_HTT/"
    )
    ...
    MET_SF_fileName = os.path.join(
        "/afs/cern.ch/work/v/vdamante/hhbbTauTauRes/prod/Framework/Studies/Triggers/files",
        "eff_Data_Mu_MC_DY_TT_WJets_mumu_metnomu_et_TRG_METNoMu120_CUTS_mhtnomu_et_L_100_default_mutau.root",
    )
    ...
    for sample_name in signal_samples + bckg_samples:
        ...
        dataframes_channel = {
            ...
            "tautau_Medium_VTight": df_tautau_VTight.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Medium.value}"
            ),
            "tautau_Medium_Tight": df_tautau_Tight.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Medium.value}"
            ),
            "tautau_Medium_Medium": df_tautau_Medium.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Medium.value}"
            ),
            "tautau_Medium_Loose": df_tautau_Loose.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Medium.value}"
            ),
            "tautau_Tight_VTight": df_tautau_VTight.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Tight.value}"
            ),
            "tautau_Tight_Tight": df_tautau_Tight.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Tight.value}"
            ),
            "tautau_Tight_Medium": df_tautau_Medium.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Tight.value}"
            ),
            "tautau_Tight_Loose": df_tautau_Loose.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Tight.value}"
            ),
            ...
            "tautau_VTight_VTight": df_tautau_VTight.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.VTight.value}"
            ),
            "tautau_VTight_Tight": df_tautau_Tight.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.VTight.value}"
            ),
            "tautau_VTight_Medium": df_tautau_Medium.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.VTight.value}"
            ),
            "tautau_VTight_Loose": df_tautau_Loose.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.VTight.value}"
            ),
            ...
            "tautau_Loose_VTight": df_tautau_VTight.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Loose.value}"
            ),
            "tautau_Loose_Tight": df_tautau_Tight.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Loose.value}"
            ),
            "tautau_Loose_Medium": df_tautau_Medium.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Loose.value}"
            ),
            "tautau_Loose_Loose": df_tautau_Loose.Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Loose.value}"
            ),
        ...
        pass_MET_application = (
            "(HLT_MET && MET_region && !(singleTau_region) && ! (other_trg_region))"
        )
        pass_singleTau_application = "(HLT_singleTau && singleTau_region && !(MET_region) && ! (other_trg_region))"
        pass_other_trg_application = (
            "({} && other_trg_region  && !(MET_region) && ! (singleTau_region) )  "
        )
        pass_ditau_application = pass_other_trg_application.format("HLT_ditau")
        trg_tauTau_list_application_fullOR = [
            pass_ditau_application,
            pass_singleTau_application,
            pass_MET_application,
        ]
        trg_tauTau_list_application_ditau_singleTau = [
            pass_ditau_application,
            pass_singleTau_application,
        ]
        trg_tauTau_list_application_ditau_MET = [
            pass_ditau_application,
            pass_MET_application,
        ]
        trg_tauTau_list_application_ditau = [
            pass_ditau_application
        ]  # ratio denumerator
        ...
        for channel in dataframes_channel.keys():
            ...
            dataframes_channel[channel] = dataframes_channel[channel].Filter(
                f"tau1new_idDeepTau{deepTauYear}{args.deepTauVersion}VSe >= {Utilities.WorkingPointsTauVSe.Loose.value}"
            )
            # print(dataframes_channel[channel].Count().GetValue())
            ...
            dataframes_channel[channel] = dataframes_channel[channel].Filter(
                f"tau2new_idDeepTau{deepTauYear}{args.deepTauVersion}VSe >= {Utilities.WorkingPointsTauVSe.Loose.value}"
            )
            # print(dataframes_channel[channel].Count().GetValue())
            ...
            dataframes_channel[channel] = getTauWPWeight(
                dataframes_channel[channel], wp_tau1, wp_tau2
            )
            dataframes_channel[channel] = (
                dataframes_channel[channel]
                .Define(
                    "weights_0",
                    "weight_total*weight_TauID_new*weight_tau1_EleidSF_Central*weight_tau1_MuidSF_Central*weight_tau2_EleidSF_Central*weight_tau2_MuidSF_Central*weight_L1PreFiring_Central*weight_L1PreFiring_ECAL_Central*weight_L1PreFiring_Muon_Central",
                )
                .Define(
                    "sel_id", "int(tau1new_gen_kind == 5) + int(tau2new_gen_kind == 5)"
                )
            )
            ...
            if sample_name in signal_samples:
                ...
                dataframes_channel[channel] = dataframes_channel[channel].Filter(
                    f"sel_id == {args.bckg_sel}"
                )
            btag_string = f"{r_factor}*weight_bTagShapeSF" if args.want2b else "1"

            total_weight_string = f"{btag_string}*weights_0"
            dataframes_channel[channel] = dataframes_channel[channel].Define(
                "final_weight", total_weight_string
            )
            # print(dataframes_channel[channel].Sum('weights_0').GetValue())
            # print(dataframes_channel[channel].Sum('final_weight').GetValue())
            regions_expr = get_regions_dict_ptreordered(
                "tauTau", [190, 190], 180, False
            )
            ...
            dataframes_channel[channel] = dataframes_channel[channel].Define(
                f"weight_ditau_application",
                f" {pass_ditau_application}?  weight_tau1_TrgSF_ditau_Central*weight_tau2_TrgSF_ditau_Central : 1.f ",
            )
            dataframes_channel[channel] = dataframes_channel[channel].Define(
                f"weight_singleTau_application",
                f"""if({pass_singleTau_application}){{
            ...
            dataframes_channel[channel] = dataframes_channel[channel].Define(
                "weight_MET_application",
                f"{pass_MET_application}? getMETsf(metnomu_pt) : 1.f",
            )

            sum_weights, sum_weights_error = getSumWeights(
                dataframes_channel[channel],
                trg_tauTau_list_application_fullOR,
                f"weights_0*weight_ditau_application*weight_singleTau_application*weight_MET_application",
            )
            ...
            if (
                sample_name in signal_samples
                and sample_name not in s_values[channel].keys()
            ):
    ...
    with open(
        f"Studies/Triggers/files/pt_reordered/signals_{sampleName}{suffix}_ptReordered.json",
        "w",
    ) as f:
        json.dump(s_values, f, indent=4)
    if bckg_samples:
        with open(
            f"Studies/Triggers/files/pt_reordered/backgrounds_{bckg_names}_{args.bckg_sel}{suffix}_ptReordered.json",
            "w",
        ) as f:

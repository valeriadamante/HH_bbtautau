...
def AddEfficiencyToDict(
    df, trg_list, channel, n_initial_channel, eff_key, eff_dict, verbose=False
):
    filter_expr = f" ("
    ...
    if verbose:
        print(
            f"with {filter_expr} : n_initial{channel} = {n_initial_channel}, n_{channel} = {n_channel}, eff_{channel} = {round(eff_channel,2)}"
        )
...
if __name__ == "__main__":
    ...
    for sample_name in sample_list:
        ...
        for channel in ["tautau_medium", "tautau_loose", "tautau_vloose"]:
            AddEfficiencyToDict(
                ...
                trg_tauTau_list_noreg_diTauOnly,
                "tauTau",
                nInitial_tauTau,
                f"eff_{channel}_noreg_diTau",
                eff_tautau_bigTau,
            )
            ...
            AddEfficiencyToDict(
                dataframes_channel[channel],
                trg_tauTau_list_reg_all,
                "tauTau",
                nInitial_tauTau,
                f"eff_{channel}_regAll",
                eff_tautau_bigTau,
            )
            tautau_labels_bigTau.append(channel)
            tautau_linestyles_bigTau.append("solid")
    ...
    makeplot(
        ...
        args.deepTauVersion,
        False,
        eff_tautau_bigTau["eff_tautau_medium_noreg_diTau"],
        "_bigTauComp",
    )
    plt.show()

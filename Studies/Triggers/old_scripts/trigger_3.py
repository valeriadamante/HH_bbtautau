...
def addHistToFile(df, var, hist_name, out_file):
    ...
    model = ROOT.RDF.TH1DModel(
        hist_name, hist_name, x_bins_vec.size() - 1, x_bins_vec.data()
    )
...
if __name__ == "__main__":
    ...
    for sample_name in sample_list:
        ...
        print(
            f""" after VALIDITY tauTau region {df.Filter("other_trg_region_tauTau").Count().GetValue()}"""
        )
        addHistToFile(
            df.Filter("other_trg_region_tauTau"),
            "tau1_pt",
            f"tau1_pt_validity",
            output_file,
        )
        ...
        print(
            f""" after TRG tauTau region {df.Filter("other_trg_region_tauTau && !singleTau_region_tauTau").Count().GetValue()}"""
        )
        addHistToFile(
            df.Filter("other_trg_region_tauTau && !singleTau_region_tauTau"),
            "tau1_pt",
            f"tau1_pt_application",
            output_file,
        )
        addHistToFile(
            df.Filter("other_trg_region_tauTau && !singleTau_region_tauTau"),
            "tau2_pt",
            f"tau2_pt_application",
            output_file,
        )

        print(f""" after only HLT_ditau {df.Filter("HLT_ditau").Count().GetValue()}""")
        addHistToFile(
            df.Filter("HLT_ditau"), "tau1_pt", f"tau1_pt_HLT_diTau", output_file
        )
        addHistToFile(
            df.Filter("HLT_ditau"), "tau2_pt", f"tau2_pt_HLT_diTau", output_file
        )

        print(
            f""" after tauTau region && HLT_ditau {df.Filter(pass_diTau_tauTau).Count().GetValue()}"""
        )
        addHistToFile(
            df.Filter(pass_diTau_tauTau), "tau1_pt", f"tau1_pt_pass_diTau", output_file
        )
        addHistToFile(
            df.Filter(pass_diTau_tauTau), "tau2_pt", f"tau2_pt_pass_diTau", output_file
        )

...
def make_plot(dict_to_plot, bckgs="01", isGrav=True):
    ...
    for final_key in dict_to_plot.keys():
        ...
        new_y_values = [
            a / b
            for a, b in zip(
                dict_to_plot[final_key][f"s/sqrtB_{bckgs}"],
                dict_to_plot["tautau_Medium_Medium_VVLoose"][f"s/sqrtB_{bckgs}"],
            )
        ]
        plt.plot(
            dict_to_plot[final_key]["masses"], new_y_values, color=color, label=label
        )
        color_key += 1
        # print(color_key)

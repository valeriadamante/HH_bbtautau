...
def GetMassCut(
    ...
    global_cfg_dict,
    channels=["tauTau"],
    categories=["boosted", "inclusive", "res2b", "res1b"],
    quantile_max=0.99,
        ...
        for channel in channels:
            ...
            if cat == "boosted":
                df_cat = df_cat.Define(
                    "FatJet_atLeast1BHadron", "SelectedFatJet_nBHadrons>0"
                ).Filter("SelectedFatJet_p4[FatJet_atLeast1BHadron].size()>0")
            else:
            ...
            if wantSequential:
                df_cat_bb = df_cat.Filter(
                    f"bb_m_vis > {min_bb_mass_int} && bb_m_vis < {max_bb_mass_int}"
                )
            ...
            print(
                f"quantile max {round(quantile_max,3)} for bb mass in category {cat} and channel {channel} =  {max_bb_mass}, {max_bb_mass_int}"
            )
            print(
                f"quantile min {round(1-quantile_max,3)} for bb mass  in category {cat} and channel {channel} =  {min_bb_mass}, {min_bb_mass_int}"
            )
            print(
                f"quantile max {round(quantile_max,3)} for tt mass  in category {cat} and channel {channel} =  {max_tt_mass}, {max_tt_mass_int}"
            )
            print(
                f"quantile min {round(1-quantile_max,3)} for tt mass  in category {cat} and channel {channel} =  {min_tt_mass}, {min_tt_mass_int}"
            )
